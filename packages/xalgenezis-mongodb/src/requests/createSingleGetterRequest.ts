...
export default (settings : Settings) : Function => {
    ...
    return createRequest(<SettingsType> settings, async (req, data, onSuccess, sharedData) => {
        ...
        if (settings.userProjectionAllowed && data[settings.userProjectionInputField]) {
            ...
            if (!Array.isArray(data[settings.userProjectionInputField])) {
                await settings.onError(
                    new GenezisGeneralError(Errors.WRONG_TYPE, { field: settings.userProjectionInputField, mustBe: "Array" }), 
                    req, 
                    data, 
                    sharedData
                );
