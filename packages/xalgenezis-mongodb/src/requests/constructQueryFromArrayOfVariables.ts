...
type ConfigData = MongoDBRequestFieldInterface[];
type QueriesArray = ConfigData[];
 ...
 */
export default async (array: QueriesArray, data: any) => {
  let query: any = {};
  ...
  for (let i = 0, length = array.length; i < length; ++i) {
    ...
    for (let j = 0, length2 = configData.length; j < length2; ++j) {
      ...
      if (data[fieldData.input] != undefined) {
        query[fieldData.field] = fieldData.convertFunc
          ? fieldData.convertFunc(data[fieldData.input])
          : data[fieldData.input];
      } else {
        allGood = false;
        break;
      }
