...
export default (settings : Settings) : Function => {
    ...
    if (!settings.queryMaker) {
        if (settings.oneField) {
            settings.queryMaker = (req : Request, data : any, sharedData : any) => {
    ...
    if (!settings.onError) settings.onError = (error : Error) => { throw error; }

    if (settings.throwErrorOnNoDocumentDeleted == undefined) settings.throwErrorOnNoDocumentDeleted = true;

    return createRequest(<SettingsType> settings, async (req : Request, data : any, onSuccess : Function, sharedData : any) => {
        if (!data) throw new RequestError(400, await getMessage(settings.messageOnNoData, req));
