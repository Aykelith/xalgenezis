...
function generateQuery(fieldName: string, data: GenerateQueryDataType): any {
  ...
  if (data.value == null) {
    throw new GenezisGeneralError("'data' must contains the value", {
      data: data,
    });
  }
  ...
  if (
    ...
    throw new GenezisGeneralError("'data.value' must be an array", {
      type: data.type,
    });
  }
...
export default (
  queryData: any,
  preAggregateData: any = {}
): { searchObject: any; queryMap: { [key: string]: number } } => {
  let searchObject: any = [{ $match: preAggregateData.$match || {} }];
  let queryMap: any = { $match: 0 };

  if (preAggregateData.$sort) {
    searchObject.push({ $sort: preAggregateData.$sort });
    queryMap.$sort = searchObject.length - 1;
  }
  // TODO: Properly
  if (queryData.sort) {
    if (!preAggregateData.$sort) {
      searchObject.push({ $sort: {} });
      queryMap.$sort = searchObject.length - 1;
    }

    // TODO: Multiple fields sorting
    const field = Object.keys(queryData.sort)[0];
    searchObject[queryMap.$sort].$sort = {
      [field]: parseInt(queryData.sort[field]),
    };
  }

  if (preAggregateData.$project) {
    searchObject.push({ $project: preAggregateData.$project });
    queryMap.$project = searchObject.length - 1;
  }
  ...
  if (queryData.projection) {
    ...
    if (preAggregateData.$project)
      Object.assign(searchObject[queryMap.$project].$project, projection);
    else {
      searchObject.push({ $project: projection });
      queryMap.$project = searchObject.length - 1;
    }
  }
  ...
  if (queryData.search) {
    ...
    for (let i = 0, length = fields.length; i < length; ++i) {
      ...
      searchObject[queryMap.$match].$match[fields[i]] = generateQuery(
        fields[i],
        data
      );
    }
  ...
  if (queryData.range) {
    ...
    if (queryData.range.x) {
      if (Number.isNaN(x) || x < 0) {
        throw new GenezisGeneralError(
          "'range.x' must be a valid positive integer"
        );
      }
    } else {
      x = 0;
    ...
    if (x != 0) {
      searchObject.push({ $skip: x });
      queryMap.$skip = searchObject.length - 1;
    }

    const y = parseInt(queryData.range.y);
    if (Number.isNaN(y) || y < x) {
      throw new GenezisGeneralError(
        "'range.y' must be a valid positive integer and bigger than 'range.x'(if present)"
      );
    }

    searchObject.push({ $limit: y - x });
    queryMap.$limit = searchObject.length - 1;
  }

  if (queryData.onlyCount) {
    searchObject.push({ $count: "number" });
    queryMap.$count = searchObject.length - 1;
  }

  return { searchObject, queryMap };
};
